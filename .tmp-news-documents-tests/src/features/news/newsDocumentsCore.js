...
export function parseNewsMarkdown(markdown) {
...
export function buildNewsDocument(path, markdown) {
...
export function buildNewsDocuments(files) {
...
export function getNewsDocumentBySlug(documents, slug) {
    if (!slug) {
        return null;
    }
    return documents.find((document) => document.slug === slug) ?? null;
}
export function getAdjacentNewsDocuments(documents, slug) {
...
export function getNewsCategories(documents) {
    return Array.from(new Set(documents
        .map((document) => document.category)
        .filter((category) => Boolean(category)))).sort((left, right) => left.localeCompare(right));
}
export function filterNewsDocuments(documents, filters) {
...
function splitExcerptAndBody(content) {
