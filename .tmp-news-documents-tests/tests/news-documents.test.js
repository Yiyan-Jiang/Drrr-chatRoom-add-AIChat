...
import { buildNewsDocument, buildNewsDocuments, filterNewsDocuments, getAdjacentNewsDocuments, getNewsCategories, getNewsDocumentBySlug, parseNewsMarkdown, slugifyHeading, } from '../src/features/news/newsDocumentsCore.js';
const firstArticle = `---
title: First News
date: 2026-06-09
author: admin
...
test('builds documents with file slugs and sensible defaults', () => {
    const document = buildNewsDocument('../content/news/first-news.md', firstArticle);
...
test('builds sorted documents and computes previous and next documents', () => {
    ...
    assert.deepEqual(documents.map((document) => document.slug), ['first-news', 'second-news']);
...
test('filters documents by query and category', () => {
    ...
    assert.deepEqual(filterNewsDocuments(documents, { query: 'nested', category: 'all' }).map((document) => document.slug), ['first-news']);
    assert.deepEqual(filterNewsDocuments(documents, { query: 'docs', category: 'announcement' }).map((document) => document.slug), ['first-news']);
    assert.deepEqual(filterNewsDocuments(documents, { query: 'docs', category: 'missing' }), []);
});
