...
const source = readFileSync(resolve(process.cwd(), 'src/pages/subPage/NewsFeed.tsx'), 'utf8');
test('NewsFeed renders the static news document list instead of a placeholder', () => {
    assert.match(source, /from\s+['"]@\/features\/news\/newsDocuments['"]/, 'NewsFeed should read static Markdown documents from the news data module.');
    assert.match(source, /getAllNewsDocuments\(\)\.then/, 'NewsFeed should lazily load the static document list.');
    ...
    assert.match(source, /to=\{`\/news\/\$\{document\.slug\}`\}/, 'Each news card should link to the standalone detail route.');
    assert.match(source, /<Link[\s\S]*?className="group block rounded border border-zinc-800 bg-zinc-950 p-4 transition hover:border-zinc-600 hover:bg-zinc-900\/80"/, 'The full news card should be clickable, not only the title.');
    assert.doesNotMatch(source, /h-96/, 'NewsFeed should no longer be the centered placeholder screen.');
});
