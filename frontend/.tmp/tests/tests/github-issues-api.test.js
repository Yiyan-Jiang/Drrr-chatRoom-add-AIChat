...
import { mapGitHubIssueCommentList, mapGitHubIssueList, mapGitHubIssueToMessageBoardIssue, } from '../src/api/githubIssueMappers.js';
...
test('maps GitHub issue fields into message board issue fields', () => {
    ...
    assert.equal(issue.htmlUrl, 'https://github.com/Yiyan-Jiang/Drrr-chatRoom-add-AIChat/issues/7');
...
test('front end requests the backend proxy instead of GitHub directly', () => {
    const source = readFileSync(resolve(projectRoot, 'src/api/githubIssues.ts'), 'utf8');
